...
local aV = {
    ScreenGui = "screenGui", SurfaceGui = "surfaceGui", BillboardGui = "billboardGui",
    Frame = "frame", ScrollingFrame = "scrollFrame",
    TextLabel = "label", TextButton = "button", TextBox = "textBox",
    ImageLabel = "imageLabel", ImageButton = "imageButton",
    UICorner = "corner", UIStroke = "stroke", UIListLayout = "listLayout",
    UIGridLayout = "gridLayout", UITableLayout = "tableLayout",
    UIPadding = "padding", UIScale = "scale",
    UIAspectRatioConstraint = "aspectConstraint", UISizeConstraint = "sizeConstraint",
    UITextSizeConstraint = "textSizeConstraint",
    Part = "part", MeshPart = "meshPart", UnionOperation = "union",
    Model = "model", Folder = "folder", Configuration = "config",
    RemoteEvent = "remoteEvent", RemoteFunction = "remoteFunction",
    BindableEvent = "bindableEvent", BindableFunction = "bindableFunction",
    LocalScript = "localScript", Script = "script", ModuleScript = "moduleScript",
    NumberValue = "numValue", StringValue = "strValue", BoolValue = "boolValue",
    IntValue = "intValue", ObjectValue = "objValue", Color3Value = "colorValue",
    Vector3Value = "vec3Value",
    Sound = "sound", SoundGroup = "soundGroup",
    Humanoid = "humanoid", HumanoidRootPart = "rootPart",
    Attachment = "attachment", Motor6D = "motor", Weld = "weld",
    WeldConstraint = "weldConstraint", HingeConstraint = "hinge",
    BodyGyro = "bodyGyro", BodyVelocity = "bodyVelocity", BodyPosition = "bodyPosition",
    Tween = "tween", TweenInfo = "tweenInfo",
    Animator = "animator", Animation = "animation", AnimationTrack = "animTrack",
    Decal = "decal", Texture = "texture", SpecialMesh = "mesh",
    Fire = "fire", Smoke = "smoke", Sparkles = "sparkles", Explosion = "explosion",
    SelectionBox = "selectionBox", SelectionSphere = "selectionSphere",
    ClickDetector = "clickDetector", ProximityPrompt = "proximityPrompt",
    Camera = "camera", Workspace = "workspace",
}
local function aW(x, aQ, aX, aS)
    ...
    if aQ then
        ...
        elseif aV[hint] then
            baseName = aV[hint]
        elseif hint:match("^[A-Z][%w]+$") then
            baseName = hint:sub(1, 1):lower() .. hint:sub(2)
        else
            local clean = hint:gsub("[^%w_]", "_"):gsub("^%d+", "_")
            if clean ~= "" and clean ~= "_" then
                baseName = clean
            end
        end
    end
    if not baseName or baseName == "" or baseName == "var" then
        baseName = "var"
    end
    local am
    if not t.names_used[baseName] then
        am = baseName
    else
        local counter = 2
        while t.names_used[baseName .. counter] do
            counter = counter + 1
        end
        am = baseName .. counter
    end
    ...
    local eR =
        ...
        {LuraphContinue = function()
                ...
                if j(aF) == "table" and not t.registry[aF] then
                    local tblHint = aE(ba)
                    local tblName
                    if tblHint and tblHint:match("^[%a_][%w_]*$") then
                        local clean = tblHint:sub(1, 1):lower() .. tblHint:sub(2)
                        if not t.names_used[clean] then
                            tblName = clean
                        else
                            local counter = 2
                            while t.names_used[clean .. counter] do counter = counter + 1 end
                            tblName = clean .. counter
                        end
                    else
                        local counter = (t.tbl_counter or 0) + 1
                        t.tbl_counter = counter
                        tblName = "tbl" .. counter
                    end
                    t.names_used[tblName] = true
                    t.registry[aF] = tblName
                end
                return aF
